...
VENACIO_SCENARIOS = [
    ...
    {
        "name": "[베나치오/흐름B] 2개월 — 복용 불가",
        "first_input": "베나치오 먹어도 돼?",
        "answers": {
            "나이": "2개월",
        },
        "expect_ok": False,
...
GELFOS_SCENARIOS = [
    ...
    {
        "name": "[겔포스/흐름B] 성인+테트라사이클린 항생제 복용 중 — 복용 불가",
        "first_input": "겔포스 먹어도 돼?",
        "answers": {
            "나이": "30살",
            "테트라사이클린계 항생제 복용자": "네",
    ...
    {
        "name": "[겔포스/흐름B] 2개월 — 겔포스현탁액 추천(나이 금기 미해당)",
        "first_input": "겔포스 먹어도 돼?",
        "answers": {
            "나이": "2개월",
            "테트라사이클린계 항생제 복용자": "아뇨",
        ...
        "expect_ok": True,
        "expect_drug": "겔포스현탁액",  # 나이 금기가 없는 단일 성분 제형만 남음
    },
]
...
TARABUTIN_SCENARIOS = [
    ...
    {
        "name": "[타라부틴/흐름B] 유당불내증(사전 탐지) — 복용 불가",
        "first_input": "유당불내증이 있는데 타라부틴 먹어도 돼?",
        "answers": {},  # 타라부틴은 나이 금기 없음 → 사전 탐지 시 역질문 없이 바로 불가
        "expect_ok": False,
    },
]
...
DULCOLAX_SCENARIOS = [
    ...
    {
        "name": "[둘코락스/흐름B] 6세 — 둘코락스좌약 추천(성인 경구 제형 제외)",
        "first_input": "둘코락스 먹어도 돼?",
        "answers": {
            "나이": "6살",
            "장출혈 환자": "아뇨",
...
MEIKINKYU_SCENARIOS = [
    ...
    {
        "name": "[메이킨큐/흐름B] 6세 — 복용 불가",
        "first_input": "아이가 변비 같은데 메이킨큐 먹여도 될까?",
        "answers": {
            "나이": "6살",
        },
        "expect_ok": False,
    ...
    {
        "name": "[메이킨큐/흐름B] 성인+궤양성 결장염 — 복용 불가",
        "first_input": "궤양성 결장염이 있는데 메이킨큐를 먹어도 될까?",
        "answers": {
            "나이": "30살",  # 복부질환자 사전 탐지 → 나이 선행 질문 후 즉시 복용 불가
        },
        "expect_ok": False,
    },
    {
        "name": "[메이킨큐/흐름B] 성인+장폐색 — 복용 불가",
        "first_input": "장폐색 환자인데 메이킨큐 먹어도 돼?",
        "answers": {
            "나이": "30살",  # 장폐색 환자 사전 탐지 → 나이 선행 질문 후 즉시 복용 불가
        },
        "expect_ok": False,
    },
]
...
PANCOL_SCENARIOS = [
    {
        "name": "[판콜/흐름B] 8살 아이 — 판콜아이콜드시럽 추천",
        "first_input": "8살 아이인데 감기 기운이 좀 있는 것 같아. 판콜 먹여도 될까?",
        "answers": {
            # 8살 나이 사전 탐지 → 나이 역질문 없음 / 15세 미만 → 수유부·알코올 skip
            "아세트아미노펜 최대 용량": "잘 모르겠어",
            "해열진통제 복용자": "아니",
        },
        "expect_ok": True,
    ...
    },
    {
        ...
        "first_input": "내가 요즘 우울해서 약을 먹고 있어. 근데 기침이 좀 나와서 그런데 판콜 먹어도 될까?",
        "answers": {
            "나이": "18살",  # 나이는 필수 역질문
        },
        "expect_ok": False,
    ...
    {
        "name": "[판콜/흐름B] 임산부(사전 탐지) — 복합 성분으로 복용 불가",
        "first_input": "임신 3개월 됐는데 지금 콧물이 계속 나서 좀 힘들어. 집에 판콜밖에 없는데 이거 먹어도 될까?",
        "answers": {
            "나이": "32살",
            "아세트아미노펜 최대 용량": "아니",
            "알코올 복용자": "아니",
    ...
    {
        "name": "[판콜/흐름B] 알코올 복용자(사전 탐지) — 복용 불가",
        "first_input": "내가 밤에 한잔 했는데 지금 몸이 으슬으슬해. 판콜 먹어도 될까?",
        "answers": {
            "나이": "30살",
        },
        "expect_ok": False,
    ...
    {
        "name": "[판콜/흐름B] 수유부(사전 탐지) — 판콜아이 제외, 나머지 2개 추천",
        "first_input": "내가 지금 모유 수유하는 중이거든. 근데 감기 기운이 좀 심하네. 판콜뿐인데 이거 먹어도 될까?",
        "answers": {
            "나이": "30살",
            "아세트아미노펜 최대 용량": "아뇨",
        },
        "expect_ok": True,
        "expect_not_drug": "판콜아이콜드시럽",  # 수유부 금기 약 제외
    },
    {
        "name": "[판콜/흐름B] 13살+아세트아미노펜 중복 복용 — 복용 불가",
        "first_input": "13살 아이인데 감기 기운이 좀 있네. 판콜 먹어도 돼?",
        "answers": {
            "나이": "13살",  # 사전 탐지 안 될 경우 fallback
            "아세트아미노펜 최대 용량": "응 타이레놀 먹였어",
        },
        "expect_ok": False,
    },
]
...
COLDONE_SCENARIOS = [
    {
        "name": "[콜대원/흐름B] 성인 — 콜드시럽 추천, 키즈이부펜 제외",
        "first_input": "콜대원 사려고 하는데, 어제부터 콧물이 줄줄 흐르고 목도 좀 따끔거려. 괜찮아?",
        "answers": {
            "나이": "24살이야",
            "임산부": "아니 없어",
            "수유부": "안하고 있어",
        ...
        "expect_ok": True,
        "expect_drug": "콜대원콜드시럽",
        "expect_not_drug": "콜대원키즈이부펜시럽",
    },
    {
        "name": "[콜대원/흐름B] 1살 아이 — 키즈이부펜시럽 추천, 콜드시럽 나이 금기 제외",
        "first_input": "콜대원 약이 있는데 1살짜리 아이가 기침이랑 콧물이 나. 먹여도 돼?",
        "answers": {
            # 1살 사전 탐지 → 나이 역질문 없음 / 1살 → 임산부·수유부·알코올 skip
            "아세트아미노펜 최대 용량": "아니",
            "해열진통제 복용자": "안먹고 있어",
        },
        "expect_ok": True,
        "expect_drug": "콜대원키즈이부펜시럽",
        "expect_not_drug": "콜대원콜드시럽",
    },
    {
        "name": "[콜대원/흐름B] 임산부(사전 탐지) — 복합 성분으로 복용 불가",
        "first_input": "콜대원 먹으려고 하는데 임신 5개월이야. 기침이 좀 나고 코도 막혀.",
        "answers": {
            "나이": "35살",
            "아세트아미노펜 최대 용량": "안 먹고 있어",
            # 임산부 사전 탐지 → 수유부 역질문 skip
        },
        "expect_ok": False,
    },
    {
        "name": "[콜대원/흐름B] 알코올 복용(사전 탐지) — 성인+알코올 금기로 추천 불가",
        "first_input": "콜대원 사러 가려는데, 어제 저녁에 한잔 했거든. 지금 콧물이랑 기침이 나는데 먹어도 돼?",
        "answers": {
            "나이": "30살",
            "임산부": "아니 없어",
        },
        # 콜드시럽·콜드에스시럽 알코올 금기로 제외, 성인이라 키즈이부펜시럽도 제외 → 추천 불가
        "expect_ok": False,
    },
]
...
PANPIRIN_SCENARIOS = [
    {
        "name": "[판피린/흐름B] 1살 아이 — 판피린큐액 추천(나이 금기 2개 해당)",
        "first_input": "1살 아이인데 기침을 좀 하네. 판피린뿐인데 이거 먹여도 될까?",
        "answers": {
            # 1살 사전 탐지 → 나이 역질문 없음 / 15세 미만 → 알코올 skip
            "아세트아미노펜 최대 용량": "아니",
            "아스피린 천식 환자": "아니",
        },
        "expect_ok": True,
    ...
    {
        "name": "[판피린/흐름B] 임산부(사전 탐지) — 복합 성분으로 복용 불가",
        "first_input": "임신 12주차인데 감기 기운이 좀 있어. 집에 판피린밖에 없는데 이거 먹어도 돼?",
        "answers": {
            "나이": "29살",
            "아세트아미노펜 최대 용량": "아뇨",
            "알코올 복용자": "안 마셨어",
    ...
    {
        "name": "[판피린/흐름B] 성인+코막힘·콧물 — 판피린큐액·에이액 2개 추천",
        "first_input": "요즘 환절기가 그런지 코가 꽉 막혀서 숨쉬기가 불편하고 맑은 콧물이 하루 종일 흘러. 판피린 먹으면 효과가 있을까?",
        "answers": {
            "나이": "21살",
            "아세트아미노펜 최대 용량": "아뇨",
            "알코올 복용자": "안 마셨어",
        ...
        "expect_ok": True,
        "expect_drug": "판피린",   # 판피린큐액·판피린에이액 모두 "판피린" 포함
    },
]
